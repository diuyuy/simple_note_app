class AppConstants {
  static const String untitled = 'AppConstants.untitled';

  static const double appBarTextButtonFontSize = 16.0;
  static const double drawerTileFontSize = 16.0;
  static const double emptyNoteTextFontSize = 24.0;
  static const double noteCardTitleFontSize = 16.0;
  static const double noteCardDateFontSize = 14.0;
  static const double titleDefaulFontSize = 16.0;

  static const double menuAnchorMinWidth = 64.0;
  static const double menuAnchorMinHeight = 40.0;

  static const int searchMaxLength = 100;
  static const int titleMaxLength = 100;
}
